...
import { IGateway } from "../../type/gateway";
...
import { IReport, SelectedFilters } from "../../type/report";
...
export const Main: FC = () => {
    ...
    const selectedProjectName = hasSingleProject ? projects.find(({ projectId }) => projectId === finalProjectIds[0])?.name : "All Projects"
    const selectedGatewayName = hasSingleGateway ? gateways.find(({ gatewayId }) => gatewayId === finalGatewayIds[0])?.name : "All Gateways"
    

    const { projects: gatewayProjects, sum: gatewayProjectSum } = groupedGatewayReport[finalGatewayIds[0]] || {}

    const showGatewayOfProjects = hasSingleGateway && !hasSingleProject
    const showOnlyProjects = !hasSingleGateway || hasSingleProject
    return <main className="ml-20 mt-4">
        <div className="grid justify-items-start grid-cols-3 grid-rows-2">
            ...
            <div className="row-start-1 col-start-2 col-span-2 w-full pr-4 justify-end flex gap-2">
                ...
                }))} handleChange={val => setSelectedFilters(v => ({
                    ...v,
                    project: val
                }))} selectedValue={selectedFilters.project} hasAll allOptionsLabel="All Projects" />

                <Select options={gateways.map((gateway) => ({
                    label: gateway.name,
                    value: gateway.gatewayId
                }))} handleChange={val => setSelectedFilters(v => ({
                    ...v,
                    gateway: val
                }))} selectedValue={selectedFilters.gateway} hasAll allOptionsLabel="All Gateways" />

                <DatePicker placeholder="From date" dateValue={selectedFilters.fromDate} maxDate={new Date()} handleDateChange={(val) => {
                    if (!val) return;
                ...
                <DatePicker placeholder="To date" dateValue={selectedFilters.toDate} minDate={selectedFilters.fromDate} handleDateChange={(val) => {
                    if (!val) return;
                    setSelectedFilters(v => ({
                        ...v,
                        toDate: val
                ...
                <button type="button" className="text-white shadow h-10 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800" onClick={fetchReports}>Generate report</button>
            </div>
            <div className="row-start-2 col-span-1">
                Easily generate a report of your transactions
            </div>
        </div>
        {reports.length ? <div className="bg-[#F1FAFE] w-[99%] mr-12 h-screen rounded-2xl p-4">
            <div className="flex font-extrabold">
                {selectedProjectName} | {selectedGatewayName}
            </div>
            <div className="flex gap-2">
                <div className={classNames(["flex flex-col mt-8 space-y-8", {
                    "w-[55%]": showGatewayOfProjects,
                    "w-[100%]": showOnlyProjects
                }])}>
                    {finalProjectIds.map((pid, index) => {
                        const { name, projectId } = projects.find(project => project.projectId === pid) as IProject;
                        return <Accordion preExpandedIds={index === 0 ? [projectId] : []} items={[{
                            id: projectId,
                            heading: <div className="flex w-full font-extrabold">
                                <div>{name}</div>
                                <div className="ml-auto">TOTAL: {groupedReports[projectId]?.sum?.toFixed(2)} USD</div>
                            </div>,
                ...
                {showGatewayOfProjects && <div className="w-[40%]">
                    <><div className="flex items-center gap-4">
                        {gatewayProjects?.map((prj, index) => {
                            return <><p className={`w-4 h-4 rounded`} style={{
                                backgroundColor: projectColors[index]
                            }} />
                            <div>{projects.find((project) => project.projectId === prj.value)?.name}</div>
                            </>
                        })}
                    </div>
                        <PieChart data={gatewayProjects?.map((val, index) => {
